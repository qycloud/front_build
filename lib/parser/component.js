...
Component.prototype._replace = function(fileName, fileType) {
    var configs = this.configs;

    fileName = fileName.replace(/\s/g, '');
    if (fileName.indexOf('res:') === 0) {
        return configs[fileName.replace('res:', '')][fileType];
    }
...
Component.prototype._parse = function(configs) {
    ...
    _.each(configs, function(configSet, key) {
	    ...
	    _.each(javascript, function(fileName, index) {
		    ...
		    _this._replace(fileName, 'javascript')
		);
	    });
        }
...
Component.prototype.parse = function(componentName, configs) {
    var components;

    if (this.components === null) {
        this.components = this._parse(
            typeof configs === 'undefined' ? this.configs : configs
        );
    }
